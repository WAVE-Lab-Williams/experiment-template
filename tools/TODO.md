...
- [ ] A cell with instructions for setup, pointing the user to install [uv](https://docs.astral.sh/uv/), including the link
- [ ] Instructions in the setup cell for initiating a uv project from the command line and to install the pyproject dependencies
- [ ] A gentle description of the UV virtual env (including a mention that deleting the `.venv` folder will give you a clean slate)
- [ ] A cell that opens `index.html` (located in ROOT) with webbrowser. Should be a variable located very clearly in the notebook for users to modify in case the location of index changes. Use pathlib over os.path if needed. Do NOT add url parameters in this step.
