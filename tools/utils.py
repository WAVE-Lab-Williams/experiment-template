...
async def validate_schema_match(
    ...
    """
    Validate that the proposed schema column names match the existing experiment type schema.

    Compares only column names, not types. Always displays the existing schema for user reference.

    Args:
        experiment_type_name: Name of the existing experiment type
        proposed_schema: Schema definition being proposed by the user
        researcher_api_key: API key for WAVE backend
        wave_backend_url: URL of WAVE backend

    Returns:
        True if column names match

    Raises:
        SystemExit: If column names don't match, exits with informative error message
    """
    # Reserved column names that are auto-generated by WAVE (should be excluded from comparison)
    RESERVED_COLUMNS = {
        "id",
        "uuid",
        "experiment_uuid",
    ...
    async with WaveClient(api_key=researcher_api_key, base_url=wave_backend_url) as client:
        try:
            # Fetch the existing schema from the API
            response = await client.experiment_types.get_columns_by_name(
                experiment_type_name=experiment_type_name
            )

            # Extract user-defined columns (exclude reserved system columns)
            existing_columns = {}
            for column in response.get("columns", []):
                column_name = column["column_name"]
                if column_name not in RESERVED_COLUMNS:
                    existing_columns[column_name] = column["column_type"]

            # Always print existing schema for user reference
            print(f"\n🔍 Existing schema for '{experiment_type_name}' in WAVE backend:")
            for field, db_type in sorted(existing_columns.items()):
                print(f"  {field}: {db_type}")

            # Compare column names only
            existing_column_names = set(existing_columns.keys())
            proposed_column_names = set(proposed_schema.keys())

            if existing_column_names != proposed_column_names:
                print(f"\n❌ Schema mismatch detected!")
                print(
                    f"⚠️  Experiment type '{experiment_type_name}' already exists with DIFFERENT columns."
                )

                # Show differences
                missing_in_proposed = existing_column_names - proposed_column_names
                missing_in_existing = proposed_column_names - existing_column_names

                if missing_in_proposed:
                    print(f"\n📉 Columns in existing schema but NOT in your proposed schema:")
                    for field in sorted(missing_in_proposed):
                        print(f"  - {field}")

                if missing_in_existing:
                    print(f"\n📈 Columns in your proposed schema but NOT in existing schema:")
                    for field in sorted(missing_in_existing):
                        print(f"  - {field}")

                print(f"\n💡 To fix this issue, you have two options:")
                print(f"   1. Use a DIFFERENT experiment type name (and table name)")
                print(
                    f"   2. Modify your proposed schema to have the SAME columns as shown above"
                )
                print(f"\n⚠️  If you need different columns, you MUST create a new experiment type.")

                sys.exit("Schema validation failed - experiment type exists with different columns")

            print(f"✅ Column names match existing experiment type!")
            return True

        except Exception as e:
            print(f"❌ Error validating schema: {e}")
            print(
                "💡 This might indicate an API connectivity issue or the experiment type doesn't exist"
            )
            raise
